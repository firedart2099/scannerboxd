...
# A MÁGICA DA FOTO DA CENA DO PERSONAGEM (PULO DO GATO)
@app.route('/api/tmdb/character_image', methods=['GET'])
def get_character_image():
    movie_name = request.args.get('movie', '')
    char_name = request.args.get('character', '')
    
    if not TMDB_API_KEY or not movie_name: return jsonify({"url": None})
    ...
    try:
        ...
        person_id = None
        # Tenta achar o personagem exato (ignora case)
        if char_name:
            for actor in cast:
        ...
        # Se não achou (ou se a IA mandou nome esquisito), pega o protagonista
        if not person_id: person_id = cast[0]['id']

        # 3. O Pulo do Gato: Tagged Images
        ...
        if res_images.status_code == 200:
            ...
            # 4. Filtra a foto onde o Ator está DENTRO do filme exato
            for img in tagged:
                if img.get('media', {}).get('id') == movie_id:
                    return jsonify({"url": f"https://image.tmdb.org/t/p/w500{img['file_path']}"})
        
        # Fallback: Se não tem cena (filme indie ou mto velho), joga o poster recortado.
        if movie_poster:
...
def gerar_perfil():
    ...
    NÃO SEJA PUXA-SACO. É PROIBIDO elogiar demais ou usar palavras como "fascinante", "refinado" ou "sofisticado". Seja debochado e aponte as hipocrisias no gosto da pessoa, mas de forma inteligente.
    ...
    2. EMOJIS: Use exatamente de 4 a 8 emojis espalhados pelo texto. USE EXCLUSIVAMENTE ESTES: {emojis_permitidos}. É PROIBIDO criar uma lista explicando os emojis.
    3. ZERO REPETIÇÃO: Vá direto ao ponto. Não fique listando os filmes repetidamente.
    4. O PERSONAGEM: Escolha um NOME DE PERSONAGEM fictício. É PROIBIDO escolher Tyler Durden, Coringa ou Patrick Bateman. Escolha um personagem genial e justifique em apenas 2 frases na última linha do texto.
    5. ORTOGRAFIA: Não invente nomes de filmes ou traduções.
    ...
    {{ 
        ...
        "filme_referencia": "Nome do Filme do Personagem (Em Inglês)", 
        "descricao": [
            "Parágrafo 1 curto (frases com ponto final, zoando a bio e os contrastes do gosto da pessoa).", 
            "Parágrafo 2 curto (zombando das notas e dando o xeque-mate justificando o personagem escolhido em 2 frases)."
        ] 
    }}"""
...
def oraculo():
    ...
    try:
        ...
        while len(recs_finais) < 8 and tentativas_ia < 2:
            favoritos = request.json.get('favorites', [])
            blacklist_amostra = random.sample(list(blacklist_raw), min(25, len(blacklist_raw)))

            prompt = f"""Atue como curador profissional. Favoritos: {favoritos}.
            Recomende EXATAMENTE 15 filmes lado B, independentes ou esquecidos.
...
def processar_em_segundo_plano(watchlist_data, sid):
    ...
    try:
        # A MÁQUINA DE TURBO: 20 WORKERS SIMULTÂNEOS SEM FREIO
...
### 2. O Frontend (`index.html`) - Com a Renderização Inteligente Sem Clones
...
<head>
    ...
    <!-- FAVICON (As 3 bolinhas na aba do navegador!) -->
    ...
    <style>
        ...
        /* O GLASSMORPHISM PREMIUM (Estilo Tidal / Windows 11) */
        .fluent-glass {
        ...
        /* As 3 Bolinhas Minimalistas do Rodapé (Sem borda, sem fundo) */
        .bottom-nav-dot {
...
<!-- Imagem de Fundo Rotativa (MAIS CLARA E VIVA) -->
...
<header class="absolute top-6 left-6 md:top-8 md:left-8 flex items-start md:items-center gap-3 z-40 pointer-events-auto w-full justify-between pr-12 md:pr-16">
    ...
    <!-- Informações do Filme no Fundo -->
    <div id="slideshow-info" class="transition-opacity duration-500 flex flex-col items-end text-right hidden sm:flex">
            ...
            <div class="fluent-glass p-8 text-center hover:border-[#40bcf4]/50 transition-all group cursor-pointer relative overflow-hidden">
                <label for="csv-file" id="dropzone-label" class="cursor-pointer block w-full h-full">
                    <div class="w-16 h-16 bg-[#40bcf4]/10 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:scale-110 transition-all border border-[#40bcf4]/30 shadow-inner">
                        <!-- Flecha Oficial Simétrica Heroicons -->
                ...
                <div id="action-area" class="hidden mt-4 flex-col items-center gap-4 relative z-10">
                    ...
                    <button id="btn-iniciar-real" onclick="handleFileUpload()" class="w-full bg-gradient-to-r from-[#00e054] to-[#00b343] text-[#0f1115] font-black py-4 rounded-xl uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(0,224,84,0.3)] hover:shadow-[0_0_35px_rgba(0,224,84,0.6)] flex items-center justify-center gap-2 hover:scale-[1.02]">
...
<!-- Frases Globais Minimalistas Flutuantes -->
...
<div id="app-view" class="fixed inset-0 w-full h-full z-30 opacity-0 transition-opacity duration-700 hidden">
    ...
    <!-- Container do Carrossel (3 Telas) -->
    <div id="carousel-wrapper" class="flex w-[300vw] h-full transition-transform duration-500 ease-in-out pointer-events-auto">
        ...
        <div class="w-[100vw] h-full overflow-y-auto pb-32 pt-28 px-4 sm:px-8 flex flex-col items-center">
            <div class="max-w-2xl w-full space-y-6">
                <div class="fluent-glass p-8 relative flex flex-col items-center text-center border-t-[#00e054]/50 shadow-[0_10px_40px_rgba(0,224,84,0.1)]">
                    
                    <!-- Avatar Contido Blindado -->
                    <div class="w-24 h-24 rounded-full border-2 border-[#00e054] bg-[#0f1115] mb-6 flex items-center justify-center shadow-[0_0_20px_rgba(0,224,84,0.3)] shrink-0 overflow-hidden relative">
                        <svg id="profile-avatar-fallback" class="w-10 h-10 text-[#2c3440] absolute" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
                        <div id="profile-avatar" class="w-full h-full absolute inset-0 z-10"></div>
                    </div>
                    
                    <h2 class="text-2xl md:text-3xl font-black text-white tracking-tighter mb-4 leading-tight" id="roast-title">
                    ...
                    <p class="text-gray-300 text-[12px] md:text-[13px] leading-relaxed px-2 md:px-6 font-light" id="roast-desc">
                        Aguarde. O Oráculo está lendo sua alma cinematográfica.
                    </p>
                </div>
            ...
            <div class="max-w-5xl mx-auto h-full flex flex-col items-center">
                ...
                <div id="watchlist-content" class="hidden w-full space-y-6">
                    ...
                    <div class="fluent-glass p-2 shrink-0 relative flex items-center">
                        ...
                        <!-- O menu de filtros agora tem z-[999] e não some quando clica -->
                        <div class="relative shrink-0 z-[999]">
    ...
    <!-- Navegação Flutuante Rodapé (As 3 Bolinhas Minimalistas Livres) -->
...
<script>
    ...
    async function loadUserBackgrounds() {
        ...
        // Dispara todos em paralelo para ser instantâneo
    ...
    async function gerarPerfilIA() {
        try {
            ...
            if(!data.erro && data.titulo) {
                document.getElementById('roast-title').innerHTML = data.titulo;
                ...
                // O Pulo do Gato (Busca a foto exata do personagem na cena do filme!)
                try {
                    let imgRes = await fetch(`/api/tmdb/character_image?movie=${encodeURIComponent(data.filme_referencia)}&character=${encodeURIComponent(data.personagem_referencia)}`);
                    let imgD = await imgRes.json();
                    if(imgD.url) {
                        fb.classList.add('hidden');
                        av.innerHTML = `<img src="${imgD.url}" class="w-full h-full object-cover absolute inset-0 rounded-full" style="object-position: center 20%;">`;
                    }
                } catch(e) {}
            } else {
            ...
            document.getElementById('roast-title').innerHTML = "O Explorador Silencioso 🧘‍♂️";
            document.getElementById('roast-desc').innerHTML = "Opa, desculpa pae! As IAs do servidor derreteram com tanto acesso hoje e não conseguiram decifrar seu gosto peculiar.<br><br>Mas ó, deslize pra aba do lado e aproveite pra ver onde assistir sua Watchlist ali na última aba! 🍿";
        }
    }
    ...
    async function carregarOraculoInicial() {
        try {
            ...
            if(data.terror_mode) {
                document.getElementById('oracle-grid').insertAdjacentHTML('beforebegin', `
                    <div class="mb-8 fluent-glass border border-[#ff8000]/50 p-6 rounded-xl text-center mx-2 shadow-[0_0_20px_rgba(255,128,0,0.15)] animate-pulse">
                        <div class="text-4xl mb-3">🏆</div>
                        <h4 class="text-[#ff8000] font-black text-sm uppercase tracking-widest mb-2">O Terror do Letterboxd</h4>
                        <p class="text-gray-300 text-[11px] leading-relaxed">A Inteligência Artificial vasculhou a internet e percebeu que <strong class="text-white">você já assistiu a tudo</strong>. Ela suou sangue, mas não tem mais o que te recomendar. Você venceu a máquina.</p>
                    </div>
                `);
            ...
            if(data.recomendacoes && data.recomendacoes.length > 0) {
                oraculoRecomendacoesGlobais = data.recomendacoes; 
                let batch = oraculoRecomendacoesGlobais.splice(0, Math.min(4, oraculoRecomendacoesGlobais.length)); 
    ...
    async function carregarMaisSugestoes() {
        ...
        // Se já tem filmes no bolso pra preencher a tela inteira (4), cospe na hora!
        if(oraculoRecomendacoesGlobais.length >= 4) {
            let batch = oraculoRecomendacoesGlobais.splice(0, 4);
            const startIdx = document.querySelectorAll('.movie-card').length;
            renderizarOraculoCards(batch, startIdx);
            return;
        }

        // Se tem buracos (menos de 4 no bolso), vai na API pedir mais!
        ...
        try {
            ...
            if(data.erro === "RATE_LIMIT") {
                if (oraculoRecomendacoesGlobais.length > 0) {
                    let batch = oraculoRecomendacoesGlobais.splice(0, oraculoRecomendacoesGlobais.length);
                    renderizarOraculoCards(batch, document.querySelectorAll('.movie-card').length);
                } else {
                    mostrarErro("As APIs estão superlotadas. Aguarde 1 minuto e tente novamente!");
                }
            } else if(data.recomendacoes && data.recomendacoes.length > 0) {
                // Filtro Javascript Anticlone: varre pra ver se o novo lote já não tá na tela
                data.recomendacoes.forEach(novo_rec => {
                    ...
                    if (!jaExiste) oraculoRecomendacoesGlobais.push(novo_rec);
                });
    ...
    function renderizarOraculoCards(recs, startIdx) {
        ...
        recs.forEach((r, i) => {
            ...
            grid.insertAdjacentHTML('beforeend', `
                <div id="${id}" class="fluent-glass movie-card group cursor-pointer relative overflow-hidden transition-all duration-300 hover:scale-[1.02] border-2 border-transparent" onclick="toggleSelecao(${idx}, '${r.rec.replace(/'/g, "\\'")}', ${r.ano})">
                    <div id="check-${idx}" class="absolute top-2 right-2 z-50 bg-[#40bcf4] text-[#14181c] rounded-full p-1 transition-all scale-0 shadow-[0_4px_10px_rgba(0,0,0,0.8)]">
                    ...
                    <img class="fav-poster absolute inset-0 w-full h-full object-cover z-10 opacity-0 transition-opacity duration-700" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" alt="Poster">
                    
                    <div class="absolute bottom-0 left-0 w-full p-4 z-20 text-center bg-gradient-to-t from-[#0f1115] via-[#0f1115]/80 to-transparent pointer-events-none">
                        <span class="text-[#40bcf4] text-[8px] font-black uppercase border border-[#40bcf4]/30 px-1.5 py-0.5 rounded bg-[#14181c]/90 drop-shadow">${r.base}</span>
                        <h5 class="text-white font-bold text-[11px] sm:text-xs line-clamp-2 mt-1.5 drop-shadow-lg leading-tight">${r.rec}</h5>
                    </div>

                    <div class="absolute inset-0 bg-[#14181c]/95 z-30 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col p-5 text-center justify-center items-center pointer-events-none"> 
                        <p class="text-[10px] sm:text-[11px] text-gray-300 line-clamp-6 leading-relaxed font-light">"${r.desc}"</p>
                        <span class="text-[8px] text-[#40bcf4] font-black mt-auto drop-shadow-md">CLIQUE P/ SELECIONAR</span> 
                    </div>
                </div>
            `);
